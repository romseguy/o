...
function script_nav:loadHotspotDB(id)
	...
	if (hotspot ~= nil and hotspot ~= -1) then
		if (self.currentHotSpotName ~= hotspot['name']) then
			DEFAULT_CHAT_FRAME:AddMessage("NEW HOTSPOT");
	...
	DEFAULT_CHAT_FRAME:AddMessage('Could not load hotspot!');
	return false;
end
	...
	while targetObj ~= 0 do
		if (targetType == 3) then -- Unit
			if(targetObj:IsDead()) then
				...
				--if (targetObj:IsLootable() and targetObj:IsSkinnable()) then
...
function script_nav:avoidElite(range) -- Runs away if there is atleast one elite within range
	...
	local localObj = GetLocalPlayer();
	while currentObj ~= 0 do
 		if typeObj == 3 and (currentObj:GetClassification() == 1 or currentObj:GetClassification() == 2 or currentObj:GetClassification() == 4) then
