...
function script_runner:setup()
	...
	-- Add Destinations
	script_runner:addDestination("Menethil", -3740.29, -755.08, 11.9643, 0, 11);
	script_runner:addDestination("Aerie Peak", 327.81, -1959.99, 198.72);
	script_runner:addDestination("Hiri'watha", -21.24, -2804.51, 121.95);
	--script_runner:addDestination("Altar of Zul", -295.39, -3459.12, 194);
	--script_runner:addDestination("JinthaAlor", -678.757, -4018.61, 239.351);
	--script_runner:addDestination("ShadraAlor",  -464.208, -2837.23, 111.07, 0, 47);
...
function script_runner:run()
	...
	if (not localObj:HasBuff('Power Word: Shield')) then
		if (Cast('Power Word: Shield', localObj)) then
			--self.timer = GetTimeEX() + self.tic;
			return;
		end
	end
