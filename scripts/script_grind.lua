script_grind = {
	...
	lootDistance = 3,
	lootCheck = {},
	minLevel = GetLocalPlayer():GetLevel()-15,
...
function script_grind:run()
	...
	if (not localObj:IsDead() and self.paranoidOn and not IsInCombat()) then 
		if (self.paranoidOnTargeted and script_grind:playersTargetingUs() > 0) then
			...
			--	self.message = 'Paranoid, using Hearthstone...';
			--	if (IsMounted()) then DisMount(); self.waitTimer = GetTimeEX()+3000; return true; end
			--	UseItem("Hearthstone");
			--	return;
			--end
...
function script_grind:enemyIsValid(i)
	if (i ~= 0) then
		...
		if (not i:IsDead() and i:CanAttack() and not i:IsCritter()
			...
			and not (self.skipElites and (i:GetClassification() == 1 or i:GetClassification() == 2 or i:GetClassification() == 4))
			) then
			return true;
		end
	end
