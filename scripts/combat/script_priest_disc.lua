...
function script_priest:healAndBuff(targetObject, localMana)
	...
	if (not IsInCombat()) then
		if (Buff('Inner Fire', targetObject)) then
			return true;
		end
	end

	-- Renew
	--if (localMana > 40 and targetHealth < self.renewHP and not targetObject:HasBuff("Renew")) then
	if (script_priest:enemiesAttackingUs(5) > 1 and localMana > 40 and targetHealth > 50 and targetHealth < 70 and not targetObject:HasBuff("Renew")) then
	...
	-- Greater Heal
	if (script_priest:enemiesAttackingUs(5) > 1) then
		if (targetHealth < 50) then
			if (script_priest:heal('Greater Heal', targetObject)) then
				return true;
			end
		end
	elseif (targetHealth < self.greaterHealHP) then
		if (script_priest:heal('Greater Heal', targetObject)) then
			return true;
		end
	end
	...
	elseif (targetHealth < self.healHP) then
		if (script_priest:heal('Heal', targetObject)) then
			return true;
		end
	end
		...
		if (target:IsSpellInRange(spellName)) then 
			if (not IsSpellOnCD(spellName)) then 
				if (not IsAutoCasting(spellName)) then
					...
					self.waitTimer = GetTimeEX() + CastTime + 800;
					return true; 
				end 
			end 
		end 
...
function script_priest:run(targetGUID)
	...
	if (targetObj ~= 0 and targetObj ~= nil) then
		...
		if (not IsInCombat()) then
			...
			if (script_priest:cast('Holy Fire', targetObj)) then
		...
		-- Combat
			...
			-- Check: Remove disease
			if (localObj:HasDebuff('Fevered Fatigue')) then
				if(HasSpell('Abolish Disease') and localMana > 40) then
					if (Buff('Abolish Disease', localObj)) then 
			...
			--if (script_priest:enemiesAttackingUs(5) > 1 and targetHealth > 20 and localMana > 40) then
			...
			if (localMana > 30 and targetHealth > 40 and not targetObj:HasDebuff("Shadow Word: Pain")) then
				if (script_priest:cast('Shadow Word: Pain', targetObj)) then
					self.waitTimer = GetTimeEX() + 200;
					return 0; 
				end
			end
			...
			-- if (self.useWand and localObj:HasRangedWeapon()) then
			-- 	if (script_priest:enemiesAttackingUs(5) > 1 and localHealth > 50) then
			-- 		if (targetHealth <= 5 or localMana <= 30) then
			...
			-- 	if (localHealth > 50 or localMana < 10) then
			-- 		if ((localMana <= 15 or targetHealth <= 15)) then
			-- 			self.message = "Using wand...";
			-- 			if (not IsAutoCasting("Shoot")) then
			-- 				targetObj:FaceTarget();
			-- 				targetObj:CastSpell("Shoot");
...
function script_priest:rest()
	...
	-- Check: Keep resting
	if((localMana < 90 and IsDrinking()) or (localHealth < 98 and IsEating())) then
